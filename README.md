...
想提前看看能抽到什么：`/老婆卡池` 查看卡池概况（收录角色/作品数、收藏 Top10），`/老婆卡池 <页码>`（如 `/老婆卡池 2`）按页浏览全量名单（收藏降序，翻页时自动附带当页「立绘墙」缩略图，每页人数可在插件 WebUI 设置「卡池浏览 → 每页人数」调整），`/老婆卡池 <角色名/别名>` 查单个角色（含作品、收藏、CV、立绘是否就绪），`/老婆卡池 作品 <作品名>` 按作品查收录阵容。立绘没就绪的角色发 `/老婆立绘 <名字>` 会尝试即时下载。
...
| `/老婆移除 <名字>` | 移除后宫成员（打入冷宫） |
| `/老婆请回 <名字>` | 花积分请回冷宫中的成员 |
| `/老婆卡池 [页码]` | 查询老婆卡池：概况 / 翻页浏览 / 查角色 / 按作品查 |
...
- ✨ **卡池翻页浏览**：`/老婆卡池 <页码>`（如 `/老婆卡池 2`）直接翻页，纯数字参数跳过角色名搜索；每页按收藏降序显示，带立绘就绪标记与收藏数，页码越界时提示范围。
- ✨ **立绘墙**：翻页时当页角色缩略图自动拼成一张图随列表发送——立绘完整显示不裁切，缺立绘的格子保留占位标记，名字显示在格子下方。
- ⚙️ **新增设置项「每页人数」**：面板「卡池浏览 → 每页人数」可调（10~200，默认 50），同时控制文字列表与立绘墙每页人数。
- 🗑 **删除 `/老婆立绘管理` 指令**：补图统一走 `/老婆立绘 <名字>` 按需下载。
- 📦 版本号全文件同步至 3.4.1。
