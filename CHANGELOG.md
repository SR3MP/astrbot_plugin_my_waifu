...
1. **`/老婆卡池 <页码>` 直接翻页浏览**
   - **背景**：原 `/老婆卡池` 只有一页展示，卡池上千角色无法完整浏览；带文字参数会被当作角色名搜索。
   - **实现**：纯数字参数（如 `/老婆卡池 2`）直接翻到对应页，跳过角色名搜索；每页按收藏数降序，显示序号、立绘就绪标记与收藏数；页码越界时提示页码范围（1 ~ N）。

2. **立绘墙**
   - 翻页浏览时，当页角色缩略图自动拼成一张「立绘墙」图片附在列表后发送。
   - 立绘按原比例缩进格子内（contain）完整显示，不裁头裁脚；缺立绘的格子保留灰色占位标记。
   - 角色名显示在格子下方，超长自动截断。

3. **新增设置项「每页人数」**
   - 面板「卡池浏览 → 每页人数」可配置（`POOL_PAGE_SIZE`，范围 10~200，默认 50）。
   - 同时控制文字列表与立绘墙的每页人数；默认 50 人/页，文字约 1100 字，主流平台单条消息安全。

### 移除

1. **删除 `/老婆立绘管理` 指令**：不再需要单独的管理入口，缺图统一走 `/老婆立绘 <名字>` 按需下载补图。

### 其他

1. **版本号全文件同步至 3.4.1**：`api_client.py` 的 `VERSION`（Bangumi API 请求 UA）、`main.py` `@register` 与文档字符串、`metadata.yaml`、`README.md` 标题统一更新。`bangumi.py` / `api_client.py` 文档字符串中「v3.4.0 重构」等历史描述性文字按语义保留，不改动。
